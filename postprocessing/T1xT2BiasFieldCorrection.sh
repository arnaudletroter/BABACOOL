...
extract_base_name() {
  ...
  if [[ -n "$SUFFIX" && "$FILENAME" == *_"$SUFFIX" ]]; then
...
T1name=$(extract_base_name "$T1" "T1w")
#T2name=`extract_base_name $T2`
T2name=$(extract_base_name "$T2" "T2w")
...
echo Normalizing...
# "${FSLPREFIX}fslmaths" ${TMP[T1mulT2]} -div $meanintval ${TMP[T1mulT2_norm]}
